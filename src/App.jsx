...
export default function SPRDApp() {
  ...
  function showFirstDayCompletePopup() {
    setPopup({
      type: "success",
      title: "First-day pages complete",
      message: "You have completed the pages needed for Victoria's first day. You will return to this document later in the SPL week to complete the midpoint work. For now, you can close the sPRD, return to Blackboard and enter the code below in the relevant box.",
      code: "1244"
    });
  }

  function showSupervisorTaskCompletePopup() {
    setPopup({
      type: "success",
      title: "sPRD task complete",
      message: "You have successfully supported Victoria as a Practice Supervisor during her placement. Her assessor, Daniel, will complete all remaining documentation at the final interview stage. You can now close Victoria's sPRD, return to Blackboard and enter the code below in the relevant box.",
      code: "1267"
    });
  }
  ...
  function showHelpPopup() {
    if (!initialPagesComplete) {
      setPopup({
        ...
        message: "When a new student starts, you need to make sure the Learning Environment, Orientation and Initial Interview pages are complete and signed by you as the Practice Supervisor, or by the assessor where appropriate. Once these first-day pages are complete, you will receive the first Blackboard code."
      });
      return;
    }
    ...
    if (!supervisorTaskComplete(record)) {
      setPopup({
        type: "help",
        title: "What do I need to do at midpoint?",
        message: "You have now been asked to complete Victoria's midpoint Practice Supervisor work. At midpoint, you need to complete and sign the Mid-Point Interview and Learning Development Review page, the Professional Values in Practice page at midpoint, and the Learner reflection on meeting Professional Values page. On the Mid-Point Interview page, review Victoria's pre-filled learner reflection and then complete the supervisor boxes. You have also been asked to provide additional feedback on the Additional Records of Feedback page by Daniel. As her assessor, Daniel will then complete the rest of Victoria's documentation, including the final interview at the end of Victoria's placement. For now, your job as supervisor is to complete the midpoint Practice Supervisor work, and then you will be given the Blackboard code."
      });
      return;
    }
    ...
    setPopup({
      type: "help",
      title: "What do I need to do now?",
      message: "You have completed the Practice Supervisor work for Victoria's sPRD. Daniel will complete the final interview and remaining final documentation at the end of Victoria's placement. You can return to Blackboard and enter the final code.",
      code: "1267"
    });
  }
  ...
  function signAndLockSection(section, signaturePath) {
    ...
    const wasInitialPagesComplete = areInitialPagesComplete(record);
    const wasSupervisorTaskComplete = supervisorTaskComplete(record);
    ...
    const nowInitialPagesComplete = areInitialPagesComplete(updated);
    const nowSupervisorTaskComplete = supervisorTaskComplete(updated);

    if (!wasSupervisorTaskComplete && nowSupervisorTaskComplete) {
      showSupervisorTaskCompletePopup();
      return;
    }

    if (!wasInitialPagesComplete && nowInitialPagesComplete) {
      showFirstDayCompletePopup();
      return;
    }
...
function PopupModal({ popup, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, zIndex: 50 }}>
      <div style={{ width: "min(560px, 100%)", background: "white", borderRadius: 22, boxShadow: "0 20px 40px rgba(15,23,42,0.35)", padding: 28, position: "relative" }}>

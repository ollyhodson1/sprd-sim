...
export function initialInterviewWordRequirementsMet(record) {
  return hasMinimumWords(record.initialInterview.learningPlan, 20) && hasMinimumWords(record.initialInterview.achievementPlan, 20);
}

export function midpointWordRequirementsMet(record) {
  return hasMinimumWords(record.midpointInterview.learningNeeds, 20) && hasMinimumWords(record.midpointInterview.achievementPlan, 20);
}

export function additionalFeedbackWordRequirementsMet(record) {
  return hasMinimumWords(record.additionalFeedback.communicationFeedback, 25);
}
...
export function getSectionValidationMessage(record, section) {
  ...
  if (section === "midpointInterview" && !midpointWordRequirementsMet(record)) {
    return "To sign and save the Mid-Point Interview, both 'Learning and development needs' and 'How will these be achieved?' need more detail. As a Practice Supervisor, your feedback should give Victoria clear coaching, direction and support for the rest of placement.";
  }
  if (section === "additionalFeedback" && !additionalFeedbackWordRequirementsMet(record)) {
    return "To sign and save Additional Records of Feedback, the 'Communication / additional feedback' box needs more detail. Please give enough detail so Victoria and her assessor can understand what happened, what was observed and what this means for her learning.";
  }
  return "";
}
...
export function runTests() {
  ...
  console.assert(!/\d+\s+words/i.test(getSectionValidationMessage(firstThreeComplete, "midpointInterview")), "Test failed: midpoint validation should not reveal the word count");

  const shortInitial = cloneRecord(defaultRecord);
  shortInitial.initialInterview.learningPlan = "Too brief";
  shortInitial.initialInterview.achievementPlan = "Also brief";
  console.assert(getSectionValidationMessage(shortInitial, "initialInterview").includes("need more detail"), "Test failed: initial interview should ask for more detail");
  console.assert(!/\d+\s+words/i.test(getSectionValidationMessage(shortInitial, "initialInterview")), "Test failed: initial interview validation should not reveal the word count");

  const shortFeedback = cloneRecord(defaultRecord);
  shortFeedback.additionalFeedback.communicationFeedback = "Too brief";
  shortFeedback.additionalFeedback.nameDesignation = "Also brief";
  console.assert(getSectionValidationMessage(shortFeedback, "additionalFeedback").includes("need more detail"), "Test failed: additional feedback should ask for more detail");
  console.assert(!/\d+\s+words/i.test(getSectionValidationMessage(shortFeedback, "additionalFeedback")), "Test failed: additional feedback validation should not reveal the word count");

  const adequateFeedbackShortDesignation = cloneRecord(defaultRecord);
  adequateFeedbackShortDesignation.additionalFeedback.communicationFeedback = "Victoria communicated clearly with the patient, listened carefully to concerns, maintained professional boundaries, responded appropriately to questions, and reflected constructively on feedback during the interaction today.";
  adequateFeedbackShortDesignation.additionalFeedback.nameDesignation = "Alex, RN";
  console.assert(additionalFeedbackWordRequirementsMet(adequateFeedbackShortDesignation), "Test failed: name and designation should not have a minimum word count");
